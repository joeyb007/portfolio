...
describe('RECRUITER completeness', () => {
  it('mentions every project by name, as a strong link', () => {
    const building = RECRUITER.groups[0]
    expect(building.label).toBe("what i've been building:")
  ...
  it('mentions every blog post by title, linking to /blog/<slug>', () => {
    const writing = RECRUITER.groups[2]
    expect(writing.label).toBe('writing:')
    expect(writing.marker).toBe('■')
    for (const post of BLOG_POSTS) {
      const item = writing.items.find(i => i.strong === post.title)
      expect(item).toBeDefined()
      expect(item!.href).toBe(`/blog/${post.slug}`)
      expect(allText()).toContain(post.title)
    }
    expect(writing.items).toHaveLength(BLOG_POSTS.length)
  })

  it('orders writing newest first', () => {
    const titles = RECRUITER.groups[2].items.map(i => i.strong)
    const sorted = [...BLOG_POSTS].sort((a, b) => b.date.localeCompare(a.date)).map(p => p.title)
    expect(titles).toEqual(sorted)
  })

  it('has the authored top / previously / links', () => {
    expect(RECRUITER.name).toBe('Joseph Barbosa')
    expect(RECRUITER.intro).toContain('Winter 2027')
    expect(RECRUITER.top.map(i => i.strong)).toEqual(
      expect.arrayContaining(['University of Waterloo', 'Wilfrid Laurier University']),
    )
    expect(RECRUITER.top.map(i => i.logo)).toEqual(
      expect.arrayContaining(['waterloo.png', 'laurier.png']),
    )
  ...
  it('only references logos that exist in /public/logos', () => {
    ...
    const used = [...RECRUITER.top, ...RECRUITER.groups.flatMap(g => g.items)]
      .map(i => i.logo).filter((l): l is string => !!l)
...
describe('deriveNote', () => {
  ...
  it('gives Scholr a users note and Studeal a live note', () => {
    const items = RECRUITER.groups[0].items
